...
escaping, ordering, and format limits. Importing and re-exporting through an
...
timecode, and frame are carried in the note text.
...
| --- | --- | --- |
| DaVinci Resolve | Marker EDL | A's source timecode, or relative zero when unavailable; one-frame marker duration |
| Final Cut Pro | FCPXML | Browser clip for source A, rational source start plus relative frame, explicit DF/NDF display |
| Avid Media Composer | Marker text | Zero-based source-A relative frame on V1 |
...
2. Check every marker against the CSV's primary frame and source/relative
   ...
   count, text, source identity, and any duplicate-frame behavior. A file
