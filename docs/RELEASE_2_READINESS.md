...
events and clock policy, an exact candidate-checkout validation mode, and direct
Review commands. No new editor, accessibility or release-floor hardware
...
| Committed live Audio QC | Implement peak/true-peak and momentary/short-term loudness with explicit units, calibration, ballistics, hold/reset behavior, and presets. Validate the actual live path against trusted references, including pause/seek/replacement, channel routing, malformed media and cancellation; demonstrate bounded work during long playback. Existing offline loudness results do not satisfy this promise. See the [live-meter implementation contract](LIVE_AUDIO_METER_DESIGN.md) and [offline audio loudness](AUDIO_LOUDNESS.md). The design, bounded DSP/display, source-rate-paced suspendable decoder, lifecycle ownership, selected A/B track mapping, typed player events and clock/drift policy are implemented. The owning window session, authoritative compressed-source timestamps, worker-side ahead bound, mounted UI and live validation remain. See [meter foundation](LIVE_AUDIO_METER_DSP.md). |
...
The September 12 continuation passes 622 Debug tests with one expected opt-in
