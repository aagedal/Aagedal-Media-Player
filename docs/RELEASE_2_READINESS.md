...
panel, an exact candidate-checkout validation mode, and direct Review commands.
...
| Committed live Audio QC | Implement peak/true-peak and momentary/short-term loudness with explicit units, calibration, ballistics, hold/reset behavior, and presets. Validate the actual live path against trusted references, including pause/seek/replacement, channel routing, malformed media and cancellation; demonstrate bounded work during long playback. Existing offline loudness results do not satisfy this promise. See the [live-meter implementation contract](LIVE_AUDIO_METER_DESIGN.md) and [offline audio loudness](AUDIO_LOUDNESS.md). The design, bounded DSP/display, source-rate-paced suspendable decoder, hard 250 ms worker admission bound, bounded precise seek with generated AAC/ALAC/AC-3 checks, lifecycle ownership, selected A/B track mapping, typed player events, clock/drift policy, owning window session and mounted activating UI are implemented. Authoritative packet timestamps and representative live-path acceptance remain. See [meter foundation](LIVE_AUDIO_METER_DSP.md). |
...
The September 13 continuation passes 637 Debug tests with one expected opt-in
real-volume-exhaustion skip and passes static analysis. Release preflight stops
on the repository's bundled FFmpeg because it does not currently have a valid
Developer ID Application signature or secure timestamp. That is an open
candidate-signing gate, not completed distribution evidence.
