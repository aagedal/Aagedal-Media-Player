...
///
/// Used to keep Sparkle out of the way for Homebrew-managed installs — those
/// users should run `brew upgrade --cask aagedal-media-player` instead,
/// otherwise an in-app update would replace the bundle that brew thinks it
/// manages and cause a checksum mismatch on the next `brew upgrade`.
enum InstallSource: Equatable {
    ...
    private static func detect() -> InstallSource {
        ...
        let caskMetadataPaths = [
            "/opt/homebrew/Caskroom/aagedal-media-player",   // Apple Silicon
            "/usr/local/Caskroom/aagedal-media-player"        // Intel
        ]
