...
nonisolated enum CompareReviewReportExporter {
    ...
    static func finalCutProXML(snapshot: CompareReviewReportSnapshot) throws -> String {
        ...
        for row in snapshot.rows { _ = try markerDurationFrames(row) }
        ...
        for frame in groups.keys.sorted() {
            ...
            let (markerFrame, overflow) = snapshot.primaryStartFrame.addingReportingOverflow(frame)
            guard !overflow else {
            ...
            let note = rows.count == 1
                ? markerNote(row: rows[0], snapshot: snapshot)
                : zip(labels, rows).map { label, row in
                    "[\(label)] \(markerNote(row: row, snapshot: snapshot))"
                }.joined(separator: "\n\n")
            lines.append(
                "      <marker start=\"\(markerStart)\" duration=\"\(frameDuration)\" value=\"\(xmlAttribute(title))\" note=\"\(xmlAttribute(note))\"/>"
            )
        }
