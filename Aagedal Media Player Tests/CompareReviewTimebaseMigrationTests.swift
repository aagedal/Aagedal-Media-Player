...
final class CompareReviewTimebaseMigrationTests: XCTestCase {
    ...
    func testReadOnlyDestinationRejectsMigrationAndAllowsRetryWithoutChangingOriginalFiles() async throws {
        ...
        do {
            _ = try await store.migrateTimebases(from: f.source, to: f.destination, expectedMigration: migration)
