...
final class LoudnessPerformanceTests: XCTestCase {
    ...
    func testProductionLoudnessProfileWhenRequested() async throws {
        ...
        for (inputIndex, path) in paths.enumerated() {
            ...
            let streams = metadata.audioStreams.enumerated().filter { ($0.element.channels ?? 0) >= 6 }
            XCTAssertFalse(streams.isEmpty, "Each input must contain a stream with at least six channels")
            var measurements: [[String: Any]] = []
            for (streamIndex, stream) in streams {
                ...
                for (scope, range) in workloads {
                    ...
                    var outcome: Result<FFmpegService.LUFSResult, Error>?
                    ...
                    guard let outcome else {
                        worker.cancel()
                        await worker.value
                        XCTFail("Loudness profile exceeded 30 minutes for \(scope), stream \(streamIndex)")
                        return
                    }
                    ...
                    measurements.append([
                        "streamIndex": streamIndex, "channels": stream.channels ?? 0,
            ...
            let report: [String: Any] = [
                ...
                "multichannelStreams": streams.count, "measurements": measurements,
            ]
            let data = try JSONSerialization.data(withJSONObject: report, options: [.sortedKeys])
            let line = "LOUDNESS_PROFILE " + String(decoding: data, as: UTF8.self)
            print(line)
            let attachment = XCTAttachment(string: line)
            attachment.name = "Loudness profile — \(inputIndex) — \(url.lastPathComponent)"
